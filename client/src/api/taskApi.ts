...
const smartAssign = async (boardId: string, taskId: string) => {
    try {
        const response = await apiClient.put(`/tasks/smart-assign/${boardId}/${taskId}`);
        return response.data;
    } catch (error: any) {
        throw error.response.data;
    }
...
export const taskApi = { createTask, getTasksByBoard, updateTask, deleteTask, smartAssign, assignTask };