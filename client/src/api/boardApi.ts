...
const removeMemberFromBoard = async (boardId: string, data: any) => {
    try {
        const response = await apiClient.put(`/boards/remove-member/${boardId}`, data);
        return response.data;
    } catch (error: any) {
        throw error.response.data;
    }
...
export const boardApi = { createBoard, getBoardsByGroupId, getBoardById, updateBoard, deleteBoard, addMemberToBoard, removeMemberFromBoard, getMembersByBoard, getActionLogs };